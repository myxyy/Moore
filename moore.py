...
print(f"target_noise_scale: {target_noise_scale}")
print(f"regularity_weight: {regularity_weight}")
print(f"regularity_noise_scale: {regularity_noise_scale}")
...
class MooreModel(nn.Module):
    def __init__(self, batch_size: int, degree: int):
        super().__init__()
        self.degree = degree
        self.mask = nn.Parameter(moore_mask(degree), requires_grad=False)
        self.params = nn.Parameter(torch.randn(batch_size, degree * (degree - 1), degree * (degree - 1)))

    def forward(self) -> torch.Tensor:
        adj_mat = F.sigmoid(self.params)
...
model = MooreModel(batch_size, degree).to(device)
